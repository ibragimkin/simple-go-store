...
func TestLoadConfig_Success(t *testing.T) {
	...
	if config.KafkaConsumerTopic != "requests" {
		t.Errorf("Expected consumer topic 'requests', got %s", config.KafkaConsumerTopic)
	}

	if config.KafkaProducerTopic != "responses" {
		t.Errorf("Expected producer topic 'responses', got %s", config.KafkaProducerTopic)
	}
